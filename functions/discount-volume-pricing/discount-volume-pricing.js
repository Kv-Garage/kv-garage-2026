/**
 * Shopify Discount Function - Volume & Cart Tier Pricing with Account Modifiers
 * 
 ...
 * - Add account modifier
 * - Cap total discount at 25%
 * - Do NOT combine or stack beyond this
 */
...
/**
 * Get account modifier based on customer tags
 * @param {string[]} tags - Customer tags array
 * @returns {number} Account modifier as decimal (0.05 = 5%)
 */
function getAccountModifier(tags) {
  if (tags.includes("wholesale")) return 0.05;
  if (tags.includes("student")) return 0.03;
  return 0;
}
...
export default function run(input) {
  ...
  // Get customer tags for account modifier
  const customerTags = input.cart.buyerIdentity?.customer?.tags || [];
  const accountBonus = getAccountModifier(customerTags);
  ...
  for (const line of input.cart.lines) {
    ...
    let bestDiscount = Math.max(cartDiscount, volumeDiscount);
    
    // Add account modifier
    bestDiscount += accountBonus;

    // Cap total discount at 25%
    if (bestDiscount > 0.25) {
      bestDiscount = 0.25;
    }
